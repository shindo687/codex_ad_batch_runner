...
# 任务：为 AutoDock-Vina 增加自动微分（AD）能力

## 目标软件

上游仓库：`https://github.com/ccsb-scripps/AutoDock-Vina`（分子对接程序：
配体-受体打分与构象搜索；核心计算在 C++，通过编译产物/绑定暴露给 Python）。
本项目要交付的是：一个与上游分开安装的 sidecar 包，为 AutoDock-Vina 中适合
求导的公共 API 提供统一 AD 接口（JVP / VJP / 梯度），并给出可复查的验收证据。
...
1. 本仓库根目录下只允许两个内容树：
   - `upstream/` —— 上游 AutoDock-Vina 的只读快照。导入方式：克隆
     `https://github.com/ccsb-scripps/AutoDock-Vina`，**删除克隆目录里的
     `.git`**，把其中全部文件放入 `upstream/`；在 `vina_ad/requirements.md`
     里记录源 commit 号、克隆时间与删除 .git 的事实。
   - `vina_ad/` —— 你的 sidecar 包；新代码只能写在这里和 `tests/`。
...
- 先公共 API 后工作流：遍历目标 API、确定 AD 范围，先写实现规格说明
  （`vina_ad/spec.md`），再实现；不能拿少量示例工作流冒充完整范围。
- 有限差分 / 复步长只允许作为测试基准（oracle）；正式规则缺失时必须明确
  报错或声明未支持，不得悄悄地用数值差分冒充已实现的 AD。
- 所有“完成”声明都要附：文件位置、环境、命令、测试数量、通过/失败数、
  误差阈值、仍未覆盖的接口清单。

## 本任务特有的范围提醒（Vina 是 C++ 核心）

- 上游打分与搜索本体是 C++，Python 侧往往只有编译产物的薄绑定，**不要
  尝试在本任务里编译整个 AutoDock-Vina C++ 工程**（Boost 依赖重、耗时长），
  也绝不改动上游源码。
- 先如实调查：选定的可微接口在 Python 侧有没有可调用的计算本体？如果只
  有“一次给整个输入、返回一个数”的二进制/Python 绑定（即无从注册规则），
  允许的方案是：在 sidecar 中提供**忠于上游公式、文档中明确标注为复刻**
  的 Python 可微内核，并且用真实 Vina 二进制/Python API 的输出做 oracle
  对照（正式运行仍必须走真实规则，oracle 仅用于测试）。
  - 复刻范围、公式来源、与真实 Vina 输出的一致性测试和偏差证据，必须写进
    `vina_ad/spec.md` 与支持状态表；证据不足的能力按“未支持”标记，不得冒充。
- 若发现“必须修改上游源码 / 改变原公共 API / 改变数据结构”的情况：停下来，
...
4. 全新 venv 里 `pip install -e vina_ad` 之后，公开接口可导入、一个真实工作流
   示例可运行，且不依赖源码目录内部路径。
5. 仓库干净：无 `__pycache__`、`*.pyc`、凭据、缓存文件入库；`git status` 干净。

## 注意

- 这是真实科学研究任务，规模远大于玩具示例：先读上游的公共 API 与文档再定范围，
  时间是几分钟到几小时级别都不要跳步。
