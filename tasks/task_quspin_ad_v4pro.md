...
# 任务：为 QuSpin 增加自动微分（AD）能力

## 目标软件

上游仓库：`https://github.com/QuSpin/QuSpin`（量子多体系统模拟库：
哈密顿量构造、时间演化、本征系统计与纠缠度量；Python + NumPy/SciPy，
部分核心用 Cython 扩展编译）。
本项目要交付的是：一个与上游分开安装的 sidecar 包，为 QuSpin 中适合求导的
...
1. 本仓库根目录下只允许两个内容树：
   - `upstream/` —— 上游 QuSpin 的只读快照。导入方式：克隆
     `https://github.com/QuSpin/QuSpin`，**删除克隆目录里的 `.git`**，把其中
     全部文件放入 `upstream/`；在 `quspin_ad/requirements.md` 里记录源 commit
     号、克隆时间与删除 .git 的事实。
   - `quspin_ad/` —— 你的 sidecar 包；新代码只能写在这里和 `tests/`。
...
- 先公共 API 后工作流：遍历目标 API、确定 AD 范围，先写实现规格说明
  （`quspin_ad/spec.md`），再实现；不能拿少量示例工作流冒充完整范围。
...
- 安装并使用上游 QuSpin 需要编译其 Cython 扩展；本机系统 Python 是 3.14.6，
  上游若未支持新版本，请用 Python 3.12 的 venv 完成安装并在证据里记录。
- 上游构建可能耗时数分钟，属正常；不要把构建产物、虚拟环境提交进仓库。
- SciPy 提供的求解器（e.g. 谱分解、求根）若本身无规则可用，把它们列为
  “暂缓/未支持”并在 spec 里写明，不要用数值差分冒充。
...
4. 全新 venv 里 `pip install -e quspin_ad` 之后，公开接口可导入、一个真实工作流
   示例可运行，且不依赖源码目录内部路径。
